...
const generateLocalReply = (q, found) => {
    const safeQ = q.trim();
    if (found.length) {
        const top = found.slice(0, 3).map((b, i) => `${i + 1}. ${b.title}${b.authors ? ` — ${Array.isArray(b.authors) ? b.authors.join(', ') : b.authors}` : ''}`).join('; ');
        ...
        if (GEMINI_KEY) {
            try {
                ...
                const booksText = books.length
                    ? books.map((b, i) => `${i + 1}. ${b.title} — ${Array.isArray(b.authors) ? b.authors.join(', ') : b.authors}`).join('\n')
                    : '';
                ...
                const resp = await globalThis.fetch(geminiUrl, {
                    method: 'POST',
                    headers: {
